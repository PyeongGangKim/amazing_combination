                ...
                children: [
                  ...
                  GetX<TagController>
                    (
                      ...
                      builder: (TagController tagController){
                        return IconButton(
                          icon : (combination.likePerson.contains(userController.user.id)) ? Icon(
                            Icons.favorite,
                            color: Colors.red,
                          ) : Icon(Icons.favorite_border),
                          iconSize: 30,
                          onPressed: (){
                            if(combination.likePerson.contains(userController.user.id)){
                              combination.likePerson.remove(userController.user.id);
                              combination.like--;
                              Get.find<CombinationController>().updateLike(combination);
                            }
                            else{
                              combination.likePerson.add(userController.user.id);
                              combination.like++;
                              Get.find<CombinationController>().updateLike(combination);
                            }
                          },
